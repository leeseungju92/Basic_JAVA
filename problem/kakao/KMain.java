package problem.kakao;

public class KMain {
public static void main(String[] args) {
//	지금 보고 있는 경기는 조별리그가 진행 중인데, 대한민국이 속한 조는
//	총 4개 국가가 참가하여 경기가 진행되고 있다.
//
//	조별리그의 규칙은 다음과 같다.
//
//	4개의 팀이 조별리그를 진행한다.
...
//	조별리그를 모두 치른 후 승점 순으로 순위를 정하는데 승점이 같을 시에는 추첨으로 순위를 정하며, 추첨은 공평하게 진행된다. 
//	순위를 정한 후 상위 2팀은 다음 라운드로 진출한다.
//	전문가들은 조별 리그의 6경기 전체에 대해서 어떤 팀이 승리할 확률, 비길 확률, 패배할 확률을 예측하였다.
//	무지와 콘은 모든 경기가 독립적으로 진행되었을 때 (어떠한 경기의 결과가 다른 경기의 결과에 영향을 주지 않음),
//	전문가들의 예상대로 진행된다면 각 팀이 조별리그를 통과하여 다음 라운드로 진출할 확률이 궁금해졌다.
...
//	두 번째 줄부터 일곱 번째 줄까지는 A B W D L 순으로 주어지는데, 전문가들의 예측에 따르면 A와 B가 경기를 진행했을 때 
//	A가 승리할 확률은 W, 비길 확률은 D, 질 확률은 L이라는 의미이다.
